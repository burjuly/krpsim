...
def get_stocks_from_processes(processes):
    stocks_in_processes = []
    for k in processes:
        tmp1 = re.split(':', processes[k][0])[0]
        tmp2 = re.split(':', processes[k][1])[0]
        stocks_in_processes.append(tmp1)
        stocks_in_processes.append(tmp2)
...
#def check_cycle(p, cycle_counter, processes)
